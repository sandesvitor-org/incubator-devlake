...
func makeDataSourcePipelinePlanV200(
	...
	for i, bpScope := range bpScopes {
		...
		pokemonType, scopeConfig, err := scopeHelper.DbHelper().GetScopeAndConfig(connection.ID, bpScope.Id)
		if err != nil {
			return nil, err
		}
		// construct task options for pokemon
		op := &tasks.PokemonOptions{
			ConnectionId:    pokemonType.ConnectionId,
			PokemonTypeId:   pokemonType.Name,
...
func makeScopesV200(bpScopes []*plugin.BlueprintScopeV200, connection *models.PokemonConnection) ([]plugin.Scope, errors.Error) {
		...
		if utils.StringsContains(scopeConfig.Entities, plugin.DOMAIN_TYPE_TICKET) {
			scopeTicket := &ticket.Board{
				DomainEntity: domainlayer.DomainEntity{
					Id: didgen.NewDomainIdGenerator(&models.PokemonType{}).Generate(connection.ID, pokemonType.Name),
				},
